...
def save_all_data():
    ...
    items = [
        ...
        (
            "daily_sessions",
            DAILY_SESSIONS_FILE,
            st.session_state.get("daily_sessions", {})
        ),
        (
            "admission_exams",
            EXAMS_FILE,
            st.session_state.get("admission_exams", [])
        ),
        (
            "timer_logs",
            TIMER_FILE,
            st.session_state.get("timer_logs", [])
        ),
        (
            "songs",
            SONGS_FILE,
            st.session_state.get("my_songs", DEFAULT_SONGS)
        ),
        (
            "syllabus",
            SYLLABUS_FILE,
    ...
    st.session_state["last_cloud_save"] = (
...
current_day_name = now_bd.strftime(
    "%A"
)
...
if st.session_state.get(
    ...
    st.session_state.page = (
        st.session_state["_bottom_nav_target"]
    )
    ...
    if st.session_state.is_focus_running:
        ...
        if not st.session_state.admission_exams:
            ...
            for ex in sort_admission_exams(st.session_state.admission_exams):
                ...
                # =================================================
                # COUNTDOWN
                # Countdown is based on EXAM DATE
                # =================================================
                with e_c5:
...
if not SUPABASE_AVAILABLE:
    ...
    if st.session_state.get(
        ...
        st.sidebar.caption(
            f"Last sync: "
            f"{st.session_state['last_cloud_save']}"
        )

else:

    st.sidebar.error(
        "🔴 Supabase is not responding"
    )

    st.sidebar.caption(
        st.session_state.get(
            "supabase_health_message",
