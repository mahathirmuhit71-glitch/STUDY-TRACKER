...
# These names are kept so the rest of the original app
# can continue using the same variables.
TASKS_FILE = "tasks.json"
SYLLABUS_FILE = "syllabus.json"
TIMER_FILE = "timer_logs.json"
DAILY_SESSIONS_FILE = "daily_sessions.json"
EXAMS_FILE = "admission_exams.json"
...
DATA_KEYS = {
    ...
    "timer_logs.json": "timer_logs",
    "daily_sessions.json": "daily_sessions",
    "admission_exams.json": "admission_exams",
    "songs.json": "songs",
}
...
def get_data_key(file_path):
    ...
    return DATA_KEYS.get(filename, filename)


def load_data(file_path, default_val):
    """
    Load permanent data from Supabase.

    Existing Supabase data is NEVER replaced by the default value.
    This means future code updates will not erase existing data.
    """

    data_key = get_data_key(file_path)
    ...
    try:
        ...
        st.error(f"Supabase data load problem: {e}")
        return default_val


def save_data(file_path, data):
    """
    Permanently save/update data in Supabase.
    """
    ...
    try:
        ...
        st.error(f"Supabase save problem: {e}")
...
HSC_DEFAULT_SYLLABUS = {
    ...
    "Physics (1st & 2nd Paper)": {
        "Chapters": {
            ...
            "Physics 1st: Chapter 1: ভৌত জগত ও পরিমাপ":
                {col: False for col in CHECKBOX_COLUMNS},
            ...
            "Physics 1st: Chapter 7: পর্যায়বৃত্ত গতি":
                {col: False for col in CHECKBOX_COLUMNS},

            "Physics 1st: Chapter 8: তরঙ্গ":
                {col: False for col in CHECKBOX_COLUMNS},

            "Physics 1st: Chapter 9: গ্যাসের গতি তত্ত্ব":
                {col: False for col in CHECKBOX_COLUMNS},
            ...
            "Physics 2nd: Chapter 5: তড়িৎ চৌম্বকীয় আবেশ ও পরিবর্তী প্রবাহ":
                {col: False for col in CHECKBOX_COLUMNS},
            ...
            "Physics 2nd: Chapter 9: পরমাণুর মডেল ও নিউক্লীয় পদার্থবিজ্ঞান":
                {col: False for col in CHECKBOX_COLUMNS},
    ...
    "Chemistry (1st & 2nd Paper)": {
        "Chapters": {
            ...
            "Chemistry 1st: Chapter 1: ল্যাবরেটরি নিরাপদ ব্যবহার":
                {col: False for col in CHECKBOX_COLUMNS},

            "Chemistry 1st: Chapter 2: গুণগত রসায়ন":
                {col: False for col in CHECKBOX_COLUMNS},

            "Chemistry 1st: Chapter 3: মৌলের পর্যাবৃত্ত ধর্ম ও রাসায়নিক বন্ধন":
                {col: False for col in CHECKBOX_COLUMNS},
    ...
    "Biology (1st & 2nd Paper)": {
        "Chapters": {
            ...
            "Biology 1st: Chapter 4: অনুজীব":
                {col: False for col in BIOLOGY_COLUMNS},

            "Biology 1st: Chapter 9: উদ্ভিদ শারীরতত্ত্ব":
                {col: False for col in BIOLOGY_COLUMNS},

            "Biology 2nd: Chapter 1: প্রাণীর বিভিন্নতা ও শ্রেণিন্যাস":
                {col: False for col in BIOLOGY_COLUMNS},

            "Biology 2nd: Chapter 3: পরিপাক ও শোষণ":
                {col: False for col in BIOLOGY_COLUMNS},

            "Biology 2nd: Chapter 4: রক্ত ও রক্তসংবহন":
                {col: False for col in BIOLOGY_COLUMNS},
...
if "daily_sessions" not in st.session_state:
    ...
    st.session_state.daily_sessions = load_data(
        DAILY_SESSIONS_FILE,
        {}
    )


if "admission_exams" not in st.session_state:

    st.session_state.admission_exams = load_data(
        EXAMS_FILE,
        []
    )
...
if "my_songs" not in st.session_state:
    ...
    st.session_state.my_songs = load_data(
        SONGS_FILE,
        DEFAULT_SONGS
    )
...
if "timer_logs" not in st.session_state:
    ...
    st.session_state.timer_logs = load_data(
        TIMER_FILE,
        []
    )
...
today_date_str = now_bd.strftime(
    "%Y-%m-%d"
)

formatted_display_date = now_bd.strftime(
    "%d %B %Y"
)


# ============================================================
# HSC TARGET COUNTDOWN
# ============================================================
...
if st.session_state.page == "🏠 Dashboard & Focus Station":
    ...
    st.markdown(
        ...
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 20px;">
            <a href="#target-tracker"
               style="text-decoration: none; font-size: 32px; line-height: 1; cursor: pointer;">
               ⚡
            </a>

            <h1 style="margin: 0; font-size: 2.25rem; font-weight: 700; color: #FFFFFF;">
    ...
    with left_col:
        ...
        if not st.session_state.tasks:
            ...
            st.info(
                "Vault is empty. Add tasks below."
            )
    ...
    with right_col:
        ...
        if st.session_state.active_focus_task:
            ...
            st.info(
                "🔥 Fixed 25-Minute Focus Session in Progress. "
                "Stay locked in!"
            )
            ...
            st.info(
                ...
                "in your Daily Planner to begin your "
                "25-minute session instantly."
            )
                        ...
                        for col_idx, col_name in enumerate(
                            ...
                            with cols[col_idx]:
                                ...
                                chk_val = st.checkbox(
                                    col_name,
                                    value=val,
                                    key=f"chk_{sub}_{ch}_{col_name}"
                                )
            ...
            with tabs[idx]:
                ...
                if sub_report_data:
                    ...
                    if REPORTLAB_AVAILABLE:
                        ...
                        def generate_subject_pdf(
                            ...
                            for _, row in dataframe.iterrows():
                                ...
                                if len(parts_list) >= 2:
                                    ...
                                    clean_ch = (
                                        f"{parts_list[0].strip()}: "
                                        f"{parts_list[1].strip()}"
                                    )
                        ...
                        pdf_bytes = generate_subject_pdf(
                            sub_name,
                            df_sub
                        )
                    ...
                    st.info(
                        f"No chapters completed yet for "
                        f"'{sub_name}'. Check off items above "
                        f"to generate your report and PDF."
                    )
    ...
    if st.session_state.is_focus_running:
        ...
        if not st.session_state.admission_exams:
            ...
            for ex in st.session_state.admission_exams:
                ...
                # Added Countdown column
                e_c1, e_c2, e_c3, e_c4, e_c5, e_c6 = st.columns(
                    [2, 1.25, 1.25, 1.25, 1.25, 0.6]
                )
                ...
                with e_c6:
                    ...
                    if st.button(
                        ...
                        st.session_state.admission_exams = [
                            item
                            for item
                            in st.session_state.admission_exams
                            if item["id"] != ex["id"]
                        ]
        ...
        with st.form(
            ...
            with f_col1:
                ...
                uni_name_input = st.text_input(
                    "University Name (যেমন: Dhaka University / BUET)"
                )
